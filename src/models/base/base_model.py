...
class BaseModel(pl.LightningModule):
    ...
    def _train_step(self, batch):
        *inputs, labels = self.split_batch(batch, includes_labels=True)
        ...
        # # 计算并记录训练准确率 (train_acc)
        # pred = helpers.npy(self(*inputs).argmax(axis=1))
        # labels = helpers.npy(labels)
        # train_metrics = calc_metrics(labels, pred)
        # self._log_dict(train_metrics, prefix="train_metrics")
